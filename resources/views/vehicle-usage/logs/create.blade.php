...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Log Penggunaan Kendaraan</h1>
        <div>
    ...
    <div class="row">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-1"></i> Log penggunaan kendaraan membantu melacak detail perjalanan, konsumsi BBM, dan status kendaraan secara real-time.
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Form Log Baru</h6>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('vehicle-usage.logs.store', $assignment) }}" method="POST">
                        @csrf
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select name="log_type" id="log_type" class="form-select @error('log_type') is-invalid @enderror" required onchange="toggleLogFields()">
                                    ...
                                    @foreach($logTypes as $value => $label)
                                        <option value="{{ $value }}" {{ old('log_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('log_type')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" class="form-control @error('log_datetime') is-invalid @enderror" id="log_datetime" name="log_datetime" value="{{ old('log_datetime', now()->format('Y-m-d\TH:i')) }}" required>
                                @error('log_datetime')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3" id="refuel-fields" style="display: none;">
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.1" min="0" class="form-control @error('fuel_added') is-invalid @enderror" id="fuel_added" name="fuel_added" value="{{ old('fuel_added') }}">
                                @error('fuel_added')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="number" step="100" min="0" class="form-control @error('fuel_cost') is-invalid @enderror" id="fuel_cost" name="fuel_cost" value="{{ old('fuel_cost') }}">
                                @error('fuel_cost')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.1" min="0" class="form-control @error('fuel_level') is-invalid @enderror" id="fuel_level" name="fuel_level" value="{{ old('fuel_level') }}">
                                @error('fuel_level')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3" id="fuel-level-field">
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.1" min="0" class="form-control @error('fuel_level') is-invalid @enderror" id="general_fuel_level" name="fuel_level" value="{{ old('fuel_level') }}">
                                @error('fuel_level')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="alert alert-warning" id="departure-info" style="display: none;">
                            <i class="fas fa-exclamation-triangle me-1"></i> <strong>Log Keberangkatan</strong>: Akan menyimpan odometer awal dan mengubah status menjadi 'Sedang Digunakan'.
                        </div>
                        
                        <div class="alert alert-warning" id="arrival-info" style="display: none;">
                            <i class="fas fa-exclamation-triangle me-1"></i> <strong>Log Kedatangan</strong>: Akan menyimpan odometer akhir. Jangan lupa untuk selesaikan penggunaan kendaraan setelah ini jika perjalanan telah berakhir.
                        </div>
                        
                        <div class="alert alert-warning" id="refuel-info" style="display: none;">
                            <i class="fas fa-exclamation-triangle me-1"></i> <strong>Log Pengisian BBM</strong>: BBM yang ditambahkan akan otomatis ditambahkan ke total penggunaan BBM kendaraan.
                        </div>
